...
findmnt /
O comando findmnt / foi utilizado para verificar onde a partição raiz está montada.
A partição principal do sistema estava montada em / utilizando o filesystem ext4.

Evidência

2. Investigação do uso de espaço
...
Antes da limpeza:
Uso: 7%
Usado: aproximadamente 29 GB

Depois da limpeza:
Uso: 5%
Usado: aproximadamente 21 GB
...
Durante a investigação, foi identificado que o Snap mantinha algumas revisões antigas dos pacotes instalados.
Para verificar as versões instaladas e antigas:
snap list --all
As revisões antigas apareciam com a indicação:
disabled
Essas revisões foram removidas utilizando o próprio Snap, através de:
sudo snap remove <nome> --revision=<revisão>
Após a limpeza, uma nova verificação foi realizada:
snap list --all
As revisões antigas disabled foram removidas.

6. Abordagem de troubleshooting
A investigação seguiu uma sequência baseada em evidências:
df -h
    ↓
...
9. Evidências
As evidências deste laboratório estão disponíveis em:
screenshots/
Capturas:
- exercicio-01-disco-e-filesystems.png
- exercicio-02-investigacao-uso-disco.png
